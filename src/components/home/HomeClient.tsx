...
// Partie interactive de l'accueil (state machine des slides + écrans auth).
// La session (connectedSpace/firstName) est désormais résolue côté serveur
...
/* ── Logo géant, écran de choix + écrans auth uniquement ──
   ...
   sautait quand même D'UN ÉCRAN À L'AUTRE (99px vs 130px, mesuré). Corrigé
   en sortant le logo du bloc centré, ancré par un `pt-*` fixe sur le
   conteneur plutôt que par `justify-center` (position identique au pixel
   près sur les 3 écrans, quel que soit le contenu).
   `pt-8` initial isolait trop le logo (grand vide avant le titre) — chaque
   écran a ensuite reçu son propre bloc `flex-1 justify-center` pour
   recentrer SON contenu indépendamment, ce qui a rétabli exactement le
   bug ci-dessus. Réglage final : ancre fixe descendue à `pt-24` (toujours
   hors du bloc centré, donc toujours identique sur les 3 écrans) et
   contenu qui suit directement en flux normal (`mt-6`, pas de flex-1) —
   logo et contenu forment un groupe visuel unique sans jamais recoupler
   la position du logo à la hauteur du contenu. */
function InlineLogo() {
  return (
...
export default function HomeClient({
  ...
  connectedSpace: string | null;
  firstName: string | null;
}) {
  // Connecté → toujours slide 3 (bandeau), déconnecté → toujours slide 0
  // (carousel complet) — décidé au premier rendu, plus de vérification
  // client au montage (voir page.tsx, résolu côté serveur).
  const [slide, setSlide] = useState<number>(connectedSpace ? 3 : 0);
    ...
    return (
      <div className="flex flex-col min-h-dvh">
        ...
        <div className="relative flex flex-col items-center flex-1 px-4">
          ...
          <div className="pt-24 relative z-10"><InlineLogo /></div>
