...
export async function GET(req: NextRequest) {
  ...
  // processBatch (lib/cron-batch.ts) isole chaque utilisateur — un
  // sendEmail() qui échoue (API down, adresse invalide...) ne doit plus
  // arrêter le traitement des utilisateurs suivants. Avant ce correctif, un
  // déclassement déjà commité en base pour l'utilisateur N pouvait être
  // suivi d'un throw sur son email qui empêchait tout traitement pour
  // N+1..fin — silencieux, sans même un 500 explicite côté cron (Vercel
  // Cron n'alerte pas sur le contenu de la réponse).
  const result = await processBatch(
    users || [],
    'verifier-inactivite',
    (u) => `${u.name || u.id} (${u.statut})`,
    async (user) => {
      if (!user.derniere_activite) return;
