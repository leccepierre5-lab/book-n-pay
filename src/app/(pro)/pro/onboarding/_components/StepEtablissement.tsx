...
export default function StepEtablissement({
  ...
  const [openTime, setOpenTime] = useState(initialOpenTime ?? '09:00');
  const [closeTime, setCloseTime] = useState(initialCloseTime ?? '19:00');
  ...
  const handleSave = async () => {
    ...
    if (!openTime || !closeTime) {
      setError('Renseignez les horaires d\'ouverture et de fermeture.');
      return;
    }
    if (openTime >= closeTime) {
  ...
  const handleUpload = async (file: File) => {
    if (photos.length >= 5) { setError('Maximum 5 photos atteint'); return; }
    setUploading(true);
    setError('');
    ...
    try {
      ...
      else setPhotos((prev) => [...prev, data.photo]);
  ...
  const handleDelete = async (photoId: string) => {
    ...
    else { const d = await res.json(); setError(d.error || 'Erreur suppression'); }
  };

  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-xl font-semibold text-white">{bizName}</h2>
        <p className="mt-1 text-sm text-slate-400">
          Indiquez vos horaires pour que vos clients puissent réserver.
        </p>
      </div>

      {/* Jours d'ouverture */}
      <div className="rounded-2xl border border-white/[0.08] bg-navy-900/60 px-5 py-5">
        <h3 className="text-sm font-semibold text-white mb-3">Jours d'ouverture <span className="text-red-400">*</span></h3>
      ...
      {/* Horaires */}
      <div className="rounded-2xl border border-white/[0.08] bg-navy-900/60 px-5 py-5">
        <h3 className="text-sm font-semibold text-white mb-3">Horaires <span className="text-red-400">*</span></h3>
        <div className="flex gap-4">
          <div className="flex-1">
            ...
            <input
              ...
              className="w-full rounded-xl bg-white/[0.06] border border-white/[0.08] px-3 py-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-mint-500/40"
            />
          </div>
          <div className="flex-1">
      ...
      <div className="rounded-2xl border border-white/[0.08] bg-navy-900/60 px-5 py-5">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-white">
            Photos <span className="text-slate-500 font-normal text-xs ml-1">({photos.length}/5 — optionnel)</span>
          </h3>
          {photos.length < 5 && (
            <button
              onClick={() => fileRef.current?.click()}
              disabled={uploading}
              className="text-xs text-mint-400 hover:text-mint-300 disabled:opacity-50 font-medium"
            >
        ...
        {photos.length === 0 ? (
          <button
            onClick={() => fileRef.current?.click()}
            disabled={uploading}
            className="w-full h-24 rounded-xl border border-dashed border-white/[0.12] flex flex-col items-center justify-center gap-2 text-slate-500 hover:text-slate-300 hover:border-white/20 transition-colors"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
              <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="17 8 12 3 7 8" /><line x1="12" y1="3" x2="12" y2="15" />
            </svg>
            <span className="text-xs">{uploading ? 'Upload en cours...' : 'Ajouter des photos (max 5 Mo)'}</span>
          </button>
        ) : (
          <div className="grid grid-cols-3 gap-2">
            ...
            {photos.length < 5 && (
              <button
                onClick={() => fileRef.current?.click()}
                disabled={uploading}
                className="aspect-square rounded-xl border border-dashed border-white/[0.12] flex items-center justify-center text-slate-500 hover:text-slate-300 transition-colors"
              >
      ...
      <div className="rounded-2xl border border-white/[0.08] bg-navy-900/60 px-5 py-5">
        <h3 className="text-sm font-semibold text-white mb-3">
          Liens & réseaux <span className="text-slate-500 font-normal text-xs ml-1">(optionnel)</span>
        </h3>
