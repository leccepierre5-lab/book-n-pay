...
// réserver quelque part, c'est avec son compte particulier." Élargi le
// 25/07 au rôle admin (même raisonnement : un compte admin gère Book'nPay,
// il ne réserve jamais de prestation réelle avec ce compte) après un audit
// navbar qui a trouvé le trou — l'ancien isProAccount() ne testait que
// role==='pro', un admin passait le check sans jamais être visé.
//
...
// confrère avec un compte pro/admin. Élargir ce check au chemin démo
// casserait la capacité de Pierre à démontrer le parcours client en
// direct : son compte de test (lecce.pierre5@gmail.com) est à la fois
// propriétaire de la vitrine démo ET whitelisté DEMO_TESTER_EMAILS.
// Messages distincts par rôle (le pro gère "son établissement", l'admin
// gère Book'nPay — les deux textes mentent si on les échange) mais un
// seul endroit à mettre à jour si le wording ou le lien changent.
...
export const ADMIN_CANNOT_BOOK_MESSAGE =
  "Ce compte administrateur gère Book'nPay et ne peut pas servir à réserver une prestation. Crée un compte client avec une autre adresse email.";

export function bookingBlockedMessage(role: 'pro' | 'admin'): string {
  return role === 'pro' ? PRO_CANNOT_BOOK_MESSAGE : ADMIN_CANNOT_BOOK_MESSAGE;
}

// Retourne le rôle bloquant ('pro'|'admin') si ce compte n'a pas le droit
// de réserver une prestation réelle en tant que client, null sinon (client
// ou compte inconnu). Une seule lecture de `role`, un seul point de vérité
// pour bookings/create, bookings/create-group et le masquage cosmétique
// de la fiche établissement.
export async function getBookingBlockedRole(
  supabase: { from: (table: string) => any },
  userId: string
): Promise<'pro' | 'admin' | null> {
  const { data } = await supabase
    .from('app_users')
    .select('role')
    .eq('id', userId)
    .maybeSingle();
  return data?.role === 'pro' || data?.role === 'admin' ? data.role : null;
}
