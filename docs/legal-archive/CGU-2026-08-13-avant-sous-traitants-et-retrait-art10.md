# CGU/CGV Book'nPay — archive avant ajout des sous-traitants (Art. 7) et retrait de l'article 10 (Favoris et notifications)

**Statut** : version en vigueur du 26/07/2026 (retrait de la section "Réservations de
groupe", voir `CGU-2026-07-26-avant-retrait-groupe.md`) au 13/08/2026. Remplacée à cette
date par deux changements distincts, issus de l'audit conformité LOT 7 :
- Article 7 (Données personnelles) : ajout de la liste des sous-traitants (Vercel, Supabase,
  Stripe, Resend) — absente jusqu'ici de toute page légale du site alors que Stripe et
  Resend traitent des données personnelles réelles (paiement, email).
- Article 10 (Favoris et notifications) : **retiré intégralement**. Décrivait une
  fonctionnalité de notification (nouveaux créneaux/promotions sur les favoris) jamais
  implémentée dans le code — recherche exhaustive confirmée sans résultat lors de l'audit.
  Une CGU ne doit pas promettre un service inexistant plutôt que de le documenter tel quel.
  Articles 11 à 15 renumérotés 10 à 14 en conséquence (sous-points inclus).

**Pourquoi cette archive** : cette version reste la version **opposable** pour tout compte
créé ou toute réservation effectuée avant le 13/08/2026 — en cas de litige portant sur
cette période, c'est ce texte qui s'applique, pas la version courante du site. Conservée
telle quelle (source : `src/app/(public)/cgu/page.tsx`, avant modification par le commit
qui introduit ce fichier), sans reformulation.
...
**5.2. Frais de gestion :** les frais de gestion ne sont pas remboursés lorsque
l'annulation émane du Client, quelle qu'en soit la date, le service de mise en relation
ayant été rendu. Le cas de l'annulation par le Professionnel est régi par l'article 3.
...
gestion des réservations et ne sont jamais revendues à des tiers. Conformément au RGPD,
...
> ⚠️ Note technique (non contractuelle, ajoutée à l'archivage) : cette version ne liste
> aucun sous-traitant (Vercel, Supabase, Stripe, Resend) — c'est précisément le manque
> corrigé par la version qui remplace celle-ci.
...
bénéficie d'une réduction de **20%** et le filleul d'une réduction de **10%** sur le prix
de leur prochaine prestation respective. Cette réduction s'applique automatiquement, est
...
> ⚠️ Note technique (non contractuelle, ajoutée à l'archivage) : recherche exhaustive dans
> le code (audit LOT 7, 13/08/2026) — aucune implémentation de cette fonctionnalité
> n'existe. La route `favorites` ne gère que l'ajout/suppression, aucun cron ni logique
> d'envoi de notification liée aux favoris n'a été trouvé. Article retiré à partir de la
> version qui remplace celle-ci plutôt que de laisser une promesse sans fonctionnalité
> derrière.
...
## 12. Litiges entre Client et Professionnel

**12.1.** Book'nPay agit exclusivement en qualité d'intermédiaire technique facilitant la
...
**12.2.** En cas de désaccord sur la qualité, le déroulement ou les modalités d'une
...
**12.3.** En cas de no-show contesté (le Client affirmant s'être présenté alors que le
...
**12.4.** Book'nPay se réserve le droit de suspendre ou de geler le compte d'un
...
## 13. Disponibilité du Service et Limitation de Responsabilité

**13.1.** Book'nPay met en œuvre des moyens raisonnables pour assurer la disponibilité et
...
**13.2.** Book'nPay ne saurait être tenu responsable des conséquences directes ou
...
**13.3.** En cas de dysfonctionnement avéré et imputable à Book'nPay ayant causé un
...
## 14. Utilisation Frauduleuse et Sécurité des Comptes

**14.1.** Toute tentative de fraude est strictement interdite, incluant notamment : la
...
**14.2.** Book'nPay se réserve le droit de vérifier, suspendre ou supprimer tout compte
...
**14.3.** L'Utilisateur est responsable de la confidentialité de ses identifiants de
...
**15.1.** La marque "Book'nPay", son logo, son identité visuelle, ainsi que l'ensemble des
...
**15.2.** Toute reproduction, représentation, modification, ou exploitation, totale ou
...
**15.3.** Les contenus publiés par les Professionnels (descriptions, photos, tarifs)
